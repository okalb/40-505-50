...
var resourceNames ={
  storageAccount: '${resourcePrefix
    }st${uniqueSuffix
    }'
  searchService: '${resourcePrefix
    }-search-${uniqueSuffix
    }'
  searchIndex: '${resourcePrefix
    }-index'
  openAiService: '${resourcePrefix
    }-openai-${uniqueSuffix
    }'
  embeddingDeployment: 'text-embedding'
}
// Ensure storage account name meets requirements (3-24 chars, lowercase alphanumeric)
var storageAccountName = length(resourceNames.storageAccount) >24 ? substring(resourceNames.storageAccount,
0,
24): resourceNames.storageAccount
// ===============================================
...
var mcpEnvBase = toLower('${resourcePrefix
}-mcp-env-${uniqueSuffix
}')
var mcpEnvName = length(mcpEnvBase) >32 ? substring(mcpEnvBase,
0,
32): mcpEnvBase

var mcpAppBase = toLower('${resourcePrefix
}-mcp-${uniqueSuffix
}')
var mcpAppName = length(mcpAppBase) >32 ? substring(mcpAppBase,
0,
32): mcpAppBase
...
var mcpImage = '${acr.properties.loginServer
}/hr-mcp-server:${mcpImageTag
}'
